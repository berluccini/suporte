...
Observe que a exceção: **ArithmeticException**  foi disparada dentro do Método **validarIdade()**, mesmo não sendo uma exceção deste tipo, exibindo a mensagem padrão do erro e o programa foi finalizado imediatamente.
...
>
> No exemplo acima, a **Classe Exception** estende (herda) a **Classe Throwable**, que implementa a **Classe Serializable**. Por isso que o Eclipse exige a inserção deste Atributo na Classe.
